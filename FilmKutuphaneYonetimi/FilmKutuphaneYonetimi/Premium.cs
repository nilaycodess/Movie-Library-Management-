    ...
    public class Premium : Kullanici
    {
        public override DataTable BilgileriGetir(int id) 
        {
            ...
            using (NpgsqlConnection connection = new NpgsqlConnection("server=localhost; port=5432; Database=nesne; user ID=postgres; password=1234"))
            {
                ...
                // id seçimine göre
                using (NpgsqlCommand command = new NpgsqlCommand("SELECT * FROM premiumbilgi WHERE id = @id", connection))
                {
        ...
        public override void BilgiEkle(int id, string adsoyad, double tc, string dogum, string cinsiyet, string tur)
        {
            using (NpgsqlConnection connection = new NpgsqlConnection("server=localhost; port=5432; Database=nesne; user ID=postgres; password=1234"))
            {
                ...
                using (NpgsqlCommand command = new NpgsqlCommand("INSERT INTO premiumbilgi (id, adsoyad, tc, dogum, cinsiyet, tur) VALUES (@id, @adsoyad, @tc, @dogum, @cinsiyet, @tur)", connection))
                {
                    command.Parameters.AddWithValue("@id", id);
        ...
        public override void BilgiGuncelle(int id, string adsoyad, double? tc, string dogum, string cinsiyet, string tur)
        {
            using (NpgsqlConnection connection = new NpgsqlConnection("server=localhost; port=5432; Database=nesne; user ID=postgres; password=1234"))
            {
                ...
                var query = new StringBuilder("UPDATE premiumbilgi SET ");
        ...
        public override void BilgiSil(string adsoyad)
        {
            using (NpgsqlConnection connection = new NpgsqlConnection("server=localhost; port=5432; Database=nesne; user ID=postgres; password=1234"))
            {
                ...
                using (NpgsqlCommand command = new NpgsqlCommand("DELETE FROM premiumbilgi WHERE adsoyad = @adsoyad", connection))
                {
                    command.Parameters.AddWithValue("@adsoyad", adsoyad);
                    command.ExecuteNonQuery();
                }
        ...
        public override void SifreSil(string adsoyad)
        {
            using (NpgsqlConnection connection = new NpgsqlConnection("server=localhost; port=5432; Database=nesne; user ID=postgres; password=1234"))
            {
                ...
                using (NpgsqlCommand command = new NpgsqlCommand("DELETE FROM premiumsifre WHERE adsoyad = @adsoyad", connection))
                {
                    command.Parameters.AddWithValue("@adsoyad", adsoyad);
                    command.ExecuteNonQuery();
                }
            ...
            try
            {
                ...
                using (NpgsqlCommand selectCommand = new NpgsqlCommand(selectQuery, baglanti))
                {
                    ...
                    using (NpgsqlDataReader reader = selectCommand.ExecuteReader())
                    {
                        ...
                        {
                            // yeni film kaydet
                            string insertQuery = @"INSERT INTO premiumliste (id, ad, yonetmen, oyuncular, tur, yayin_yili, degerlendirme) 
        ...
        public override DataTable FilmListesi()
        {
            ...
            using (NpgsqlConnection connection = new NpgsqlConnection("server=localhost; port=5432; Database=nesne; user ID=postgres; password=1234"))
            {
                ...
                using (NpgsqlCommand command = new NpgsqlCommand("SELECT * FROM premiumliste", connection))
                {
                    using (NpgsqlDataAdapter adapter = new NpgsqlDataAdapter(command))
                    {
                        adapter.Fill(dataTable);
        ...
        public override void FilmSil(string ad)
        {
            using (NpgsqlConnection connection = new NpgsqlConnection("server=localhost; port=5432; Database=nesne; user ID=postgres; password=1234"))
            {
                ...
                using (NpgsqlCommand command = new NpgsqlCommand("DELETE FROM premiumliste WHERE ad = @ad", connection))
                {
